...
class AOCDay extends AOC {
  ...
  override async evaluate(): Promise<void> {
    ...
    // With previous solution, get all combinations of the minimum number of containers that fit solution.
    for (const combination of combinations(this.store.get(), min)) {
      const totalSize = combination.reduce((i, container) => i = i + container, 0);
      // Check if totalSize is 150 and note 1 additional proper combination in final count.
      if (totalSize === 150) {
        this.count.add(1);
      }
    }
